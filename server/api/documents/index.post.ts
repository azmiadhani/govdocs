...
export default defineEventHandler(async (event) => {
  ...
  const type =
    formData.find((f) => f.name === "type")?.data?.toString() || "other";
  const ministry =
    formData.find((f) => f.name === "ministry")?.data?.toString() || "";
  const publishedAt =
    formData.find((f) => f.name === "publishedAt")?.data?.toString() || null;
  const tags = JSON.parse(
    formData.find((f) => f.name === "tags")?.data?.toString() || "[]",
  ) as string[];
